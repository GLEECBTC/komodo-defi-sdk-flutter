...
void main() {
  ...
  for (final boundary in ['reauthentication', 'same-identity replacement']) {
    test('$boundary rejects old pubkeys and accepts a fresh context', () async {
      final client = _MockApiClient();
      final auth = _MockAuth();
      final activation = _MockActivationCoordinator()
        ..stubNotFreshlyActivated();
      ...
      Map<String, dynamic> result(String address) => {
        'address': address,
        'balance': '5',
        'unspendable_balance': '0',
        'coin': asset.id.id,
      ...
      when(() => client.executeRpc(any())).thenAnswer((_) {
        ...
        return Future.value(result('cosmos1fresh'));
      });
      final manager = PubkeyManager(client, auth, activation, storage: storage);
      addTearDown(() async {
        await manager.dispose();
      ...
      final pending = manager.getPubkeys(asset);
      ...
      response.complete(result('cosmos1stale'));
      await rejected;
      expect(storage.savedWallets, isEmpty);
      expect(manager.lastKnown(asset.id), isNull);
      expect(
        (await manager.getPubkeys(asset)).keys.single.address,
        'cosmos1fresh',
      );
      expect(storage.savedWallets, [currentUser.walletId]);
    });
  }
  for (final streamed in [false, true]) {
    test(
      'reauthentication stops pending address creation (streamed: $streamed)',
      () async {
        final auth = _MockAuth();
        final client = _MockApiClient();
        final activation = _MockActivationCoordinator();
        ...
        final manager = PubkeyManager(
          ...
          auth,
          activation,
          storage: _RecordingPubkeysStorage(),
        );
        addTearDown(() async {
          await manager.dispose();
          await authChanges.close();
        });
        final Future<Object> pending = streamed
            ? manager.watchCreateNewPubkey(asset).toList()
            : manager.createNewPubkey(asset);
        final rejected = expectLater(
          pending,
          throwsA(isA<WalletChangedDisconnectException>()),
        );
        await started.future;
        auth.runtimeSessions
          ..invalidate()
          ..observe(_walletA);
  ...
  test('reauthentication rejects a pending unban response', () async {
    final auth = _MockAuth();
    final client = _MockApiClient();
    final authChanges = StreamController<KdfUser?>.broadcast();
    final response = Completer<Map<String, dynamic>>();
    ...
    final pending = manager.unbanPubkeys(const UnbanBy.all());
    final rejected = expectLater(
      pending,
      throwsA(isA<WalletChangedDisconnectException>()),
    );
    ...
    auth.runtimeSessions
      ..invalidate()
      ..observe(_walletA);
    response.complete({'result': <String, dynamic>{}});
    await rejected;
  });
}
